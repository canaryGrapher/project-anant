...
export default function ResultCard({ mxene, latticeConstant, bandGap, id, idList, setIdList }) {
    ...
    return (
        <div
            ...
            className={`w-full flex flex-col items-center justify-between md:p-8 p-4 rounded-md select-none ${idList.includes(id) ? "border shadow-xl" : ""}`}
        ...
        >
            <div className="md:w-auto w-4/5 flex flex-row items-center justify-center md:border-0 border-t-2 border-b-2 my-2 border-gray-400 md:text-left text-center">
                <p className={`text-white text-2xl md:my-0 my-4 ${idList.includes(id) ? "font-medium text-black" : ""}`}>{mxene}</p>
            </div>
