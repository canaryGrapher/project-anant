...
const PageBar = ({currPage, numberOfPages}) => {
    console.log(currPage + 1)
    console.log([...Array(numberOfPages).keys()])
    return (
        <div className="flex flex-wrap justify-center items-center">
            {
                ((currPage - 1) > 0) 
                && 
                <Link href={`/apps/mxene/filter?M1=Cr&M2=&T1=&T2=&X=&currentPage=${currPage - 1}`}>
            ...
            }
            {
                [...Array(numberOfPages).keys()].map((page) => {
                    return (
                        <Link href={`/apps/mxene/filter?M1=Cr&M2=&T1=&T2=&X=&currentPage=${page+1}`}>
                            <div 
                                key={page} 
