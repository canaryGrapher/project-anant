...
export default function MxeneFilter({ query, res }) {
    ...
    const handleDownload = async () => {
        ...
        if (idList.length > 10) {
            return MyToaster({ header: "Max download limit!", message: "You cannot download more than 10 mxenes at once" });
        }
        if (loggedIn) {
            try {
                const mxeneIds = idList.join(",");
                const res = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/downloadmxene/?id=${mxeneIds}`)
            ...
            <div className="w-full px-8 py-6">
                ...
                {res.mxenes.length > 0 ?
                    ...
                    <div className="w-full h-32 flex flex-col items-center justify-center">
                        ...
                        <p className="text-lg text-gray-300">No results were found. Try adjusting your search parameters.</p>
                        <button className="bg-white theme-text px-3 py-2 mt-5 rounded-lg font-bold hover:theme hover:text-white border-2 border-white" onClick={() => router.back()}>Try again?</button>
                    </div>
                }
